...
function parcelHTML(o){const done=orderStatus(o)==='done';const imageAction=o.imageUrl?`<a class="secondary" href="${escape(o.imageUrl)}" target="_blank" rel="noopener noreferrer">打开订单截图</a>`:`<span class="secondary disabled">无订单截图</span>`;return `<div class="parcel ${done?'done':''}"><div class="parcel-top"><span class="station">${escape(o.station||'未填驿站')}</span></div><div class="code">${escape(o.code||'未填写取件码')}</div><div class="actions">${imageAction}<button class="secondary" data-action="copy" data-id="${o.id}">复制码</button>${done?'<span class="complete locked">已取到 ✓</span>':`<button class="complete" data-action="complete" data-id="${o.id}">标记已取到</button>`}</div></div>`;}
...
app.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.tab){tab=b.dataset.tab;render();return;}if(b.dataset.station){station=b.dataset.station;render();return;}const action=b.dataset.action,id=b.dataset.id,o=orders.find(x=>x.id===id);if(action==='import')fileInput.click();if(action==='copy'&&o){navigator.clipboard?.writeText(o.code);showToast('取件码已复制');}if(action==='complete'&&o&&orderStatus(o)!=='done'){o.status='done';persist();showToast('已标记为取到，状态已锁定');render();}});
